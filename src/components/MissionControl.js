...
const MissionControl = () => {
  ...
  useEffect(() => {
    let rafCoord = null;
    let rafTelemetry = null;

    const updateTelemetry = () => {
      if (rafTelemetry) return;
      rafTelemetry = requestAnimationFrame(() => {
        rafTelemetry = null;
    ...
    const updateCoordinates = (event) => {
      ...
      rafCoord = requestAnimationFrame(() => {
        ...
        setTelemetry((current) => ({
          ...current,
          x: String(Math.round((clientX / window.innerWidth) * 359)).padStart(3, "0"),
          y: String(Math.round(90 - (clientY / window.innerHeight) * 180)).padStart(3, "0"),
        }));
      });
    };
    ...
    return () => {
      ...
      if (rafTelemetry) cancelAnimationFrame(rafTelemetry);
  ...
  return (
    <>
      <div className="celestial-backdrop" role="presentation" aria-hidden="true">
      ...
      <div className="trajectory" role="presentation" aria-hidden="true">
